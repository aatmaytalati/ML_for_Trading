...
class RTLearner(object):
    ...
    def copy_instance(self, var1): #utilizing to make a copy. Its easier to have funcation. 
        return var1.copy()

    def left_t(self, param1, param2, param3, param4): #Left Tree
        # param1 = Xtrain
        # param2 = temp_int
        # param3 = Val_to_split_on
        # param4 = Ytrain
        return self.buildtree(param1.ix[param1.ix[:,param2]<=param3],param4[param1.ix[:,param2]<=param3])
    
    def right_t (self,param1, param2, param3, param4): #Likewise, Right Tree
        return self.buildtree(param1.ix[param1.ix[:,param2]>param3],param4[param1.ix[:,param2]>param3])
    
    def check_point(): #Checking the version of python libraries
        print "Checking the version for the each Modules"
        print ("Numpy", np.__version__)
        print ("Pandas", pd.__version__)
        print ("MatPlotLib", matplotlib.__version__)
        print ("Please wait while AutoGrader runs the result:")
       # print ("If you have an import Error for a module, consider installing Anaconda in your machine")

    check_point()

    def __init__(self,leaf_size,verbose =False): #Constructor. 
        self.leaf_size = leaf_size
        
        self.verbose = verbose

    # Changing the Constructor
    # def __init__(self, leaf_size=1, verboseself=False, tree=None): #, **kwargs):
    #     # Treee initilization 
    ...
    #     #                 If not None ->    tree isself.tree = tree
    #     # tree = None a numpy ndarray. 
    ...
    def split_on_best_feature(self, dataX, dataY): #choosing a feature to split on
    ...
    def query(self,Xtest): 
        temp = self.copy_instance(Xtest) # Creating copy of the instance. 
        temp[temp.columns.values[0] +'_holding'] = temp.apply(self.s_q,axis = 0+1) # Referance: https://stackoverflow.com/questions/17071871/select-rows-from-a-dataframe-based-on-values-in-a-column-in-pandas/17071908
        temp.ix[self.minus_One(),temp.columns.values[0] +'_holding'] = self.Zer0()

        return temp[[temp.columns.values[0] +'_holding']]
    ...
    def buildtree(self,Xtrain,Ytrain):
        ...
        if Xtrain.shape[0] <= self.leaf_size:
            return [['leaf',max(temp_dict, key=temp_dict.get),'','']]
        ...
        if y_sm:
            ...
            for i in range(Xtrain.shape[1]):
                temp_lst.append(np.random.uniform())
            temp_lst_list = map(abs,temp_lst)
            temp_int = temp_lst_list.index(np.nanmax(temp_lst_list))
            boo = True
            med = np.median(Xtrain.ix[:,temp_int])
            for i in Xtrain.ix[:,temp_int]:
                if i > med:
                    boo = not (True)
            Val_to_split_on = np.median(Xtrain.ix[:,temp_int]) if (not boo) else np.mean(Xtrain.ix[:,temp_int])
	    if (len(Xtrain) == len(Xtrain.ix[Xtrain.ix[:,temp_int] <= Val_to_split_on])|(len(Xtrain) == 0)):
		return [['leaf', max(temp_dict, key=temp_dict.get),'','']]
            lefttree = self.left_t(Xtrain, temp_int, Val_to_split_on, Ytrain)
            rightree = self.right_t(Xtrain, temp_int, Val_to_split_on, Ytrain)
            root = [temp_int,Val_to_split_on,self.minus_One()+2,len(lefttree)+1]
#	    print ([root]+lefttree+righttree)
#	    print SplitVal
            return ([root]+lefttree+rightree)
            print ("exiting RTLearner")
...
#     dtl.addEvidence(x2_2, y2)
    ...
    def s_q(self,s):
        ...
        boo = True
        while (boo*1):
            if s[self.dt[temp][0]] < self.dt[temp][1] or s[self.dt[temp][0]] == self.dt[temp][1] :
                temp += self.dt[temp][2]
            else:
                temp += self.dt[temp][3]
            if self.dt[temp][0] == 'leaf':
                return self.dt[temp][1]
